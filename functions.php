...
function advanced_custom_search( $where, $wp_query ) {
    ...
    foreach( $exploded as $tag ) :
        $where .= " 
          AND (
            (emcUNCGX_posts.post_title LIKE '%$tag%')
            OR (emcUNCGX_posts.post_content LIKE '%$tag%')
            OR EXISTS (
              SELECT * FROM emcUNCGX_postmeta
                WHERE post_id = emcUNCGX_posts.ID
                  AND (";
        ...
        endforeach;
          ...
          $where .= ")
            ...
            OR EXISTS (
              SELECT * FROM emcUNCGX_comments
              WHERE comment_post_ID = emcUNCGX_posts.ID
                AND comment_content LIKE '%$tag%'
            )
            OR EXISTS (
              SELECT * FROM emcUNCGX_terms
              INNER JOIN emcUNCGX_term_taxonomy
                ON emcUNCGX_term_taxonomy.term_id = emcUNCGX_terms.term_id
              INNER JOIN emcUNCGX_term_relationships
                ON emcUNCGX_term_relationships.term_taxonomy_id = emcUNCGX_term_taxonomy.term_taxonomy_id
              WHERE (
              taxonomy = 'post_tag'
                OR taxonomy = 'category'              
                OR taxonomy = 'myCustomTax'
              )
                AND object_id = emcUNCGX_posts.ID
                AND emcUNCGX_terms.name LIKE '%$tag%'
            )
        )";
    endforeach;
    return $where;
